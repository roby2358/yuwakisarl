...
## Forced Moves
If at least one die yields a legal move, the player must make a legal move.

The player may choose the die order. The player is not required to use both dice; they may choose an order that results in only one die being played even if another order would allow both.
...
If a player has no checkers on the bar and no die exactly matches, the player may use a higher die to bear off the checker that is farthest back (i.e., farthest from bearing off: lowest-numbered point for the human, highest-numbered for the AI).
...
## Variant: Full-Entry Requirement
